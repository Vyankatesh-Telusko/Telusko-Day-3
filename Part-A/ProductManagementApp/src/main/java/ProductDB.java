...
public class ProductDB {
    ...
    public ProductDB() {
        try {
            conn = DriverManager.getConnection("jdbc:postgresql://localhost:6060/Telusko", "postgres","password");
...
*/
    public List<Product> getName(String name) {
        ...
        List<Product> products = new ArrayList<>();
        String query = "select name,type,place,warranty from product as p where p.name=?;";
        try {
            PreparedStatement st = conn.prepareStatement(query);
            st.setString(1, name);
